...
|---|---|---|
| GHCR containers are never collected | All 45 showed **Next collection: Not scheduled**, and the sweep only picks up resources with a due date. They came from the July 2026 snapshot, which set no dates | Fixed in main by [#30](https://github.com/ICICLE-ai/icicle-insights/pull/30). The migration runs when the API starts after the next deploy |
| No 30-day GHCR pulls yet | Only the lifetime totals imported on 2026-07-24 exist | Clears after the first sweep once #30 is deployed |
...
- **Take screenshots of the new dashboard for the README.** The old Angular ones were deleted.
  Capture at 1440×900, 2× scale, light theme.
